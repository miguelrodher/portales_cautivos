              ...
              <form action="{{ route('usuarios.store') }}" method="POST" novalidate>
                ...
                <div class="mb-3">
                  <input id="correo" name="correo" type="email"
                    class="form-control border border-2 border-dark rounded-1 py-2 @error('correo') is-invalid @enderror"
                    placeholder="Correo electrónico" value="{{ old('correo') }}" maxlength="150">
                  @error('correo')
                    <div class="text-danger small mt-1">{{ $message }}</div>
                  @enderror
                </div>
